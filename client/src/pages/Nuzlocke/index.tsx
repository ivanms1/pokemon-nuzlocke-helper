...
const Nuzlocke = () =>
{
  ...
  const team = nuzlocke.pokemons.filter((pok: { inTeam: boolean }) => pok.inTeam);
  const inPc = nuzlocke.pokemons.filter((pok: { inTeam: boolean, status: string }) => !pok.inTeam && pok.status !== 'DEAD');
  const deadMons = nuzlocke.pokemons.filter((pok: { status: string }) => pok.status === 'DEAD');
